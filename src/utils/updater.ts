...
export class Updater {
  ...
  dispatch: Record<string, ((...args: any[]) => void)[]>; //小型发布订阅通知用户更新了
  ...
  on(key: 'no-update' | 'update', fn: (...args: any[]) => void) {
  ...
  compare(oldArr: string[], newArr: string[]) {
    ...
    const arr = Array.from(new Set(oldArr.concat(newArr))); //如果新旧length 一样无更新
    if (arr.length === base) {
